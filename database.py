...
class log():
    ...
    def __init__(self, action, user_input, response, model_name='', status_code = '', input_type = ''):
    ...
    def queries(self) -> None:
        if self.db_connection == True:
            if self.action == 'POST':
                sql = (f'INSERT INTO log_data (Date, Time, Method, Model_Name, Input_Type, User_Input, Response, Status_Code, Status_Message, Status_Description)'
                       f' VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)')
                values = (time.strftime('%Y-%m-%d'), time.strftime('%H:%M:%S'), self.action, self.model_name, self.input_type, self.user_input, self.response, self.status_code, self.status_message, self.status_description)
